...
preference-related target conditions moreover show that our design is
able to detect multiple readings and preferences on these. Crucially,
the amount of response types for \lc- and \ec-readings reflected the
known preference for the former, irrespective of the order in which
each reading could first be judged in a given sequence. Given this,
the best and most straightforward explanation of the observed response
type frequencies in our target condition is the hypothesis that the
actual preferences among readings for both \as- and \es-sentences
alike are:
\begin{exe}
