...
theoretical positions, which we call \mymark{traditionalism} and
\mymark{grammaticalism} and take to be general ideas behind large
...
directly because additional ``auxiliary assumptions'' are needed in
both cases to derive falsifiable \emph{ex ante} predictions about
to-be-expected observations in a concrete behavioral experiment, even
irrespective of processing accounts. We therefore try to carefully set
up plausible instantiations of either core theory with particular
auxiliary assumptions that give testable predictions at the outset in
...
marked utterances, albeit not as the result of a standard scalar
implicature but of a different pragmatic process triggered by prosodic
markedness
...
e.g.][]{Bader98,Fodor98}. We will therefore address this extant
position as the \mymark{prosodic markedness hypothesis} and argue
that, if prosody is hypothesized to play a central role, it is vital
to present target sentences auditorily or to control for effects of
“silent prosody” that subjects may apply when reading a
sentence. Secondly, as \citet{Tielvan-Tiel2012:Embedded-Scalar} and
...
\emph{ex ante} predictions of the core theories traditionalism and
grammaticalism (for some practically relevant auxiliary assumptions)
about availability and relative salience of readings for our target
...
experimental design, analyses and results. Finally,
Section~\ref{sec:critical-reflection} discusses our method and
...
\mymark{grammaticalism} and treat each in turn. Both approaches are
addressed here as ``core theories'': two main ideas behind several
approaches to quantity implicatures. To determine concrete predictions
from these, additional ``auxiliary assumptions'' are necessary. Here,
we spell a set of such auxiliary assumptions for each core theory. Our
motivation for the particular choices we make is partly guided by
practical concerns, so as to have a concrete set of predictions for
each core theory to formulate expectations and compare our data to. We
therefore introduce some prominent or otherwise relevant variants of
either core theory, the predictions of which are summarized in
Table\ref{tab:predictions} on page~\pageref{tab:predictions}. How
other potential instantiations of either core theory fare in light of
our data will be discussed in Section~\ref{sec:critical-reflection}.
...
\citet{Geurts2010:Quantity-Implic} calls the \mymark{Gricean recipe}
for deriving a scalar inference (\ref{bsp:Plain-SI-Implicature}) from
an utterance of (\ref{bsp:Plain-SI-Target}): if the issue whether Hans
solved only some or all of the problems is relevant, then a
cooperative and knowledgable speaker would utter
...
weak variants, and between restricted and unrestricted
variants. 
...
or not in the given context of utterance (or some experiment or
other). These assumptions include relevance of the information in
question, mutual awareness that the stronger alternative has been a
speaker option, the speaker's competence about the issue,
etc. \emph{Strong traditionalism} assumes that the additional
assumptions needed to derive a non-epistemic implicatures are
sufficiently salient to prefer the non-epistemic implicature over the
...
traditionalism is more relaxed and also allows alternatives that are
derived from replacing \emph{some} with \emph{all} and vice versa, if
necessary multiple times, even if the resulting alternative is not
logically stronger. Restricted traditionalism considers only
\as-sentences to have a relevant alternative, namely
(\ref{bsp:AE-Alternative}). It then predicts the availability of a
global reading for \as-sentences on top of the literal. Restricted
traditionalism predicts only literal readings of \es-sentences,
because the alternative (\ref{bsp:GE-Alternative}) does not entail the
literal reading of (\ref{bsp:GE}). Every implicature reading available
to restricted traditionalism is also available to unrestricted
traditionalism, but the latter's predictions are even richer. For
\as-sentences, the potential alternative
(\ref{bsp:AE-Alternatives-Extended}) also makes local readings
available \citep[e.g.][]{Sauerland2004:Scalar-Implicat}: conjoining a
literal reading of (\ref{bsp:AE}) with the negation of
...
negation of alternative (\ref{bsp:GE-Alternative}).
...
truth-value, if necessary multiple times
\citep[see][]{Chierchia2006:Broaden-Your-Vi,Fox2007:Free-Choice-and,Magri2011:Another-Argumen,Sauerland2012:The-Computation,ChierchiaFox2008:The-Grammatical}. For
our purposes, it is enough to assume that $\exh(\cdot)$ is a poly-typed
...
approach should be supplemented with a principled \emph{disambiguation
  mechanism} so as to rule out unattested readings obtainable by
embedded exhaustification and to make concrete predictions about
preferences over readings made available. % Grammaticalists therefore
...
obligatory presence of $\exh(\cdot)$ with variability in the set of relevant
alternatives.  Under this approach, alternatives may be pruned
entirely, so that all occurrences of $\exh(\cdot)$ become vacuous resulting
in a literal reading. Whether alternatives apply or not is a matter of
their relevance. It is not clear what his approach predicts \emph{ex
  ante} about the relative salience of the candidate readings in our
experiment, but we will show in Section~\ref{sec:critical-reflection}
that this approach can be brought to accommodate the data well after
the fact.
...
Luckily, the last kind of disambiguation approach does yield a clear
and non-trivial restriction of reading preferences off-the-shelf. It
is to select readings by comparison of logical strengths, as suggested
by the \mymark{strongest meaning hypothesis} of
...
from now on this latter type of selection principle. In the discussion
section (section~\ref{sec:discussion-1}) alternative selection
principles will be reconsidered.

\citet{ChierchiaFox2008:The-Grammatical} discuss two concrete variants
of strength-based disambiguation that make non-trivial predications
...
and readings preferences for our target sentences. Most notably, under
...
Grammaticalism, when supplemented with the the strongest meaning
hypothesis as disambiguation principle, predicts that all candidate
...
\begin{table}[t]
  ...
  \caption{Predictions of the relevant theoretical positions. Listed
    ...
    readings are not ranked, i.e., not predicted to differ as opposed
    to being predicted to be equally ranked.}
