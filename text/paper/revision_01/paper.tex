...
% and domain-general reasoning procedures (i.e. pragmatics).
...
and the latter the \mymark{the weak variety of traditionalism}.\fn{We
  mention for clarity that although weak traditionalism does not
  predict literal readings to be more salient than non-epistemic
  global readings, it might still predict an \emph{epistemically weak
    implicature}, similar to the global reading, that the speaker is
  uncertain whether the stronger alternative is true. (See also the
  discussion in Section~\ref{sec:discussion-1})} It is of course an
...
\as-sentences is not available; in contrast, \mymark{unrestricted
  traditionalism} assumes that (\ref{bsp:AE-Alternatives-Extended}) is
available and so is the local reading.\fn{The term traditionalism may
  be misleading here because traditionalists usually assume that
  scalar implicatures should only be computed when a sentence is
  uttered in a context in which a logically stronger alternative is
  relevant \cite{ChemlaSingh2014:Remarks-on-the-}. However, the
  alternative in (\ref{bsp:AE-Alternatives-Extended}) is not stronger
  than (\ref{bsp:AE}). For completeness sake, we still decided to
  subsume “unrestricted traditionalism” under traditionalism because,
  once alternatives like (\ref{bsp:AE-Alternatives-Extended}) are
  acknowledged, the local reading of \as-sentences can be derived in a
  straightforward traditionalist manner \citep[see also][who call this
  type of theory ``unrestricted
  globalism'']{ChemlaSpector2010:Experimental-Ev}.}
...
Other proponents of grammaticalism assume that insertion of $\exh$ is
optional.  These approaches treat the ambiguity between our candidate
readings as a structural one which resembles syntactic ambiguities. In
order to derive predictions about reading preferences a mechanism of
ambiguity resolution is needed in these approaches. One proposal is
that readings are the more preferred the better they answer the
contextually given question under discussion
\citep[e.g.][]{Fox2007:Free-Choice-and,GualminiHulsey2008:The-Question-An}.
...
\citep[see][]{FoxSpector:Economy-and-Emb,ChierchiaFox2008:The-Grammatical,Chierchia2012:FC-Nominals-and,Spector2014:Global-Positive}. For
now, we focus exclusively on this latter selection principle, because
it seems to be the most prominent in the literature and because it
makes the clearest predictions. In the discussion section
(section~\ref{sec:discussion-1}) alternative selection principles will
be reconsidered.
...
judgments: a sentence like \emph{Some of the 10 marbles are white} is
more typical, intuitively speaking, in a situation with 4 white
marbles than in a situation with 8 or 9 white marbles \citep[for more
on the controversial interpretation of ``typicality'' in this context
see][]{Tielvan-Tiel2014:Quantity-Matter,Cummins2014:Typicality-made,
  ,Franke2014:Typical-use-of-}. \citeauthor{ChemlaSpector2010:Experimental-Ev}
show that graded judgments of \as-sentences differ significantly for
different pictures that agree on the truth value of relevant readings
but differ with respect to the amount of connections between
...
accommodate the observed data. \todo{look at reviewer comment and
  acknowledge duly.} 
