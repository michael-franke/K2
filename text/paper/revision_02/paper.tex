...
should reflect on the effects of ``silent prosody.''
...
unambiguously map responses, ideally categorical, to specific readings. (ii) We should try to
avoid issues of ``silent prosody'' and actively explore the role of prosodic markedness. (iii)
We would like to obtain information about the relative preferences of the different readings,
ideally by comparison to some other, ``base-line'' condition. Finally (iv) we want to minimize
potential picture typicality effects.
...
\paragraph{General Idea.} To deal with these desiderata, we used an \emph{incremental
  verification task} (\acro{ivt}), which is a modified version of picture verification
\citep[see][]{Conroy2008}. The general idea is that subjects are requested to judge sentence
material with respect to pictures that do not necessarily contain all the information relevant
for judging a certain reading true or false. In that case, participants can demand that more
information be revealed. Participants are instructed to make a truth-value judgment as soon as
they are able to. When they do, the trial ends.
