...
readings. \citeauthor{ChemlaSpector2010:Experimental-Ev}'s results
...
truth-value judgment (\citeauthor{ChemlaSpector2010:Experimental-Ev}).
...
available, but so dispreferred that they do not affect simple
\emph{categorical} truth-value judgements. It could then furthermore
be argued, in line with traditionalism, that the reason why local
...
Either position is cast into doubt by a pilot study of
ourselves.\dn{We should expand on the method in an appendix} We tried
to replicate \citeauthor{ChemlaSpector2010:Experimental-Ev}'s results,
using a simple categorial truth-value judgement task on pictures that
were exactly like those in Figures \ref{fig:AS-distinguishing-pics}
and \ref{fig:ES-distinguishing-pics}. Test sentences were German
cleft-constructions as in (\ref{bsp:AS-K1}) and
(\ref{bsp:ES-K1}).\fn{We concede that these cleft-constructions might
  unduly emphasize potential local readings, but that is not
  essential, because the main issue we are trying to address here is
  whether putative local readings \emph{could} affect categorical
...
hand, both reached significance.\dn{spell out in appendix}
...
truth-value judgements. However, this interpretation of the data is
problematic for reasons already addressed also by
...
\end{figure}
%
Our pilot study revealed a difference in acceptability,
but a curious one. The sentence was judged \emph{true} in 90.3\% of
the cases for the situation in Figure~\ref{fig:global-EA} and in
98.3\% in situation \ref{fig:literal-EA}. The observed difference is
significant ($t = -3.8259$, $df = 322.375$, $p < 0.001$). This is a
curious finding, because, from the point of view of implicature
calculation, the only thing that could plausibly explain this
acceptability difference is the presence of a global implicature,
resulting from the negation of the corresponding
\es-sentence.\fn{Notice that (\ref{bsp:EA}) is the alternative to the
  corresponding \es-sentence needed for computing the putative global
  implicature. But these sentences are logically independent. So, it
  may be that the corresponding \es-sentence is an alternative also to
  sentences like (\ref{bsp:EA}). It may also be that being an
  alternative is not a symmetric notion. Be that as it may, our
  argument does not hinge on whether (\ref{bsp:EA}) does or does not
  actually have a global implicature of the alleged kind.} But such a
putative global implicature would be true in the
situation~\ref{fig:global-EA}, but false in the
situation~\ref{fig:literal-EA}. Consequently, the observed difference
in acceptability cannot be explained simply in terms of the hypothesis
of \citet{ChemlaSpector2010:Experimental-Ev} that acceptability is
proportional to the number of available true readings. Rather some
feature of the pictorial material that is independent of the putative
readings under scrutiny does affect truth-value judgements, even
categorical ones.
...
categorical judgements in our pilot.

To conclude, the confounding effects of pictorial complexity and/or
typicality are real and affect even categorical judgements. The
available evidence about availability, preference and nature of local
readings is therefore still inconclusive. What is needed to complement
the picture is data from a design that elicits categorical judgements
that are as independent as possible of possibly conflating properties
...
impossible to achieve. As we will argue below we do, however, believe
it is.

On top of this, we would like to obtain more further information about
