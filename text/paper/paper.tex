...
all different instantiation of the situation types in
...
\begin{figure}[t]
  \centering
      \includegraphics[scale=0.35]{../pictures/paper/Chemla_Spector_2010_Critical_AE.png}\\
      \includegraphics[scale=0.35]{../pictures/paper/Chemla_Spector_2010_RatingBar.png}
...
degree to which a sentence was rated acceptable is proportional to the
number of available true readings. Observed averaged clicking
positions are shown in Figure~\ref{fig:Chemla-Spector-Results}.
%
\begin{figure}
  \centering
...
pieces of evidence concerning putative local readings. G\&P's results
...
task. On the other hand, C\&D's and C\&S's design was suggested to be
sensitive enough to reveal effects of local readings: either by a
choice of the situation that better befits a given sentence (C\&D), or
by a graded truth-value judgment (C\&S). 

To account for all of the available data in conjunction, it could be
hypothesized along with C\&D and C\&S that local readings are
available, but so dispreferred they do not affect simple
\emph{categorical} truth-value judgements. This, however, does not
seem to be the case, as revealed by a pilot study of ourselves. We
tried to replicate G\&P's results, using a simple categorial
truth-value judgement task on pictures that were exactly like those in
Figures \ref{fig:AS-distinguishing-pics} and
\ref{fig:ES-distinguishing-pics}. Test sentences were German
...
\end{exe}
60 native German speakers (40 female, mean age: $24$, ranging between
$20$ and $40$) participated in this pilot. Participants were na\"ive
to the purpose of the study and were paid \euro{10}
compensation.\dn{We should expand on the method in an appendix}

Mean acceptance rates for our target
sentences were (see also Figure~\ref{fig:Bars-K1}):
\begin{center}
  ...
  \end{tabular}
\end{center}
Two observations are of interest here. Firstly, unlike G\&P, we
observed \emph{false}-judgements in the \as-weak condition, as well as
\emph{true}-judgements in the \es-local condition. Secondly, the
observed difference between the \as-weak and the \as-strong
conditions, on the one had, and the \es-false and the \es-local
condition, on the other hand, both reached significance.\dn{spell out
  in appendix} 
...
The perhaps most obvious explanation for these observed patterns is
that local readings \emph{are} available and \emph{can} have an effect
even on categorical truth-value judgements. However, we believe that
this interpretation of the data is problematic for reasons already
addressed also by \citet{ChemlaSpector2010:Experimental-Ev} and, more
recently, \citet{Tielvan-Tiel2012:Embedded-Scalar}. The problem is
that the present variants of truth-value judgment tasks all run risk
of conflating evidence about the availability or preference of
readings with properties of the pictorial material that are
independent of the reading to be assessed. This problem affects all of
the studies covered above, to some degree or other, because it follows
almost directly from the nature of the phenomenon under investigation
and therefore seems nearly impossible to avoid.

To illustrate the potential conflation, consider a sentence like
(\ref{bsp:EA}), which is a ``global alternatives'' to an \es-sentence,
...
\end{exe}
Sentence like (\ref{bsp:EA}) are literally true in both of the
situations given in Figure~\ref{fig:EA-distinguishing-pics}, but these
situations differ in the truth value of the corresponding
\es-sentence. Our pilot study revealed a difference in acceptability,
but a curious one. The sentence was judged \emph{true} in 90.34\% of
the cases for the situation in Figure~\ref{fig:global-EA} and in
98.34\% in situation \ref{fig:literal-EA}. The observed difference is
...
resulting from the negation of the corresponding \es-sentence. But
such a putative global implicature would be true in the
...
conditions in \ref{fig:AS-distinguishing-pics}, that differ in the
amount of connections, but not so that they would alter the assignment
of truth-values to candidate
readings. \citeauthor{ChemlaSpector2010:Experimental-Ev} suggest that
typicality of the pictorial material can account for these
differences, but submit that this does not explain away the effect of
the local reading. This is disputed by
...
obvious way\fn{For similar reasons, appeal to typicality cannot
  explain, as far as we can see, the
  preference for the situation in Figure~\ref{fig:literal-EA} over
  that in Figure\ref{fig:global-EA} given a sentence like
  (\ref{bsp:EA}).} and that it also does not explain why we observed putative
effects of typicality also in categorical judgements in our pilot.
...
typicality or other crucial properties. Given the logical dependencies
...
\item what's needed
  \begin{itemize}
  \item evidence about all preferences
  \item factor out picture-complexity
  \item preferably categorical judgements
  \item intonation
  \end{itemize}
...
\item K1:
  \begin{itemize}
  \item tried to replicate
    \citet{GeurtsPouscoulous2009:Embedded-Implic}'s study
  \item but tested all conditions
  \item used German cleft-sentences (possibly problematic)
  \item some evidence for subjective patterns in answers
  \end{itemize}
\item Thirdly, we observed that a tendency that participants
who rejected \as-sentences in the literal situation where more prone
to also accept \es-sentences in the local condition.\dn{spell out
  in appendix} This suggests that there might be participants who are
more likely to be influenced by local readings than others.
\end{itemize}
...
\subfloat[fig:literal][literal]{
  ...
  \caption{Distinguishing scenarios for sentence XYZ}
