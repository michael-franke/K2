...
\begin{exe}
\ex \label{bsp:AE-Alternatives-Extended-Crucial} \mymark{Some} of the students solved \mymark{all} of the problems.
\end{exe}

\noindent Especially (\ref{bsp:AE-Alternatives-Extended-Crucial}) is
important here. Clearly, if we conjoin a literal reading of
(\ref{bsp:AE}) with the negation of
(\ref{bsp:AE-Alternatives-Extended-Crucial}) we obtain exactly the
local reading. (Notice that the negation of
(\ref{bsp:AE-Alternatives-Extended-Crucial}) entails the negation of
...
(\ref{bsp:AE-Alternatives-Extended-Crucial}) hinges on another
auxiliary assumption, namely the availability of the alternative in
(\ref{bsp:AE-Alternatives-Extended-Crucial}), which seems much less
...
handy labels as follows:
\begin{description}
\item[Tradition$_\text{strong}$] 
\end{description}
