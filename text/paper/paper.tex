...
We also tested examples like (\ref{bsp:EA}), which are the ``global
alternatives'' to \es-sentences, with a universal quantifier where
\es-sentences have scalar \emph{some}.
\begin{exe}
\ex \gll F\"{u}r \mymark{genau} \mymark{eines} dieser Kinder gilt: es mag \mymark{jede} dieser
  Speisen. \label{bsp:EA}\\
