...
\noindent In \as-sentences as in (\ref{bsp:AE}) the quantifier
\emph{some} is embedded under universal quantifier \emph{all}. In
\es-sentences like (\ref{bsp:GE}) \emph{some} takes scope under the
non-monotonic quantifier \emph{exactly one}. These sentences are
interesting, because, according to current pragmatic theorizing, they
may have at least three conceivable readings. The main questions we
address here are, firstly, which of these three conceivable readings
are available to subjects na\"{i}ve to all pragmatic theory, and,
secondly, which of the available readings are preferred.\dn{Fabian
  says: Das brauchen wir an dieser Stelle, glaube ich,
  nicht. Vielleicht im Abstract? Mit Präferenzen wäre ich vorsichtig:
  Wir bekommen keine globalen Lesarten, wollen aber nicht sagen, dass
...
similar to logical $\exists$,\dn{petra says: quellenangaben nötig?
  micha says: this is really commonly assumed/known... wouldn't know
  who (not) to cite} so that the
sentence \emph{Some boys cried} is literally true in a situation where
all boys cried. But it is also known to invite upper-bounding
inferences in plain utterances:
...
There are then at least three relevant candidate readings for \as- and
...
\noindent Section~\ref{sec:get-know-your} will discuss these readings
...
the availability and preferences of readings. We will give a more
detailed description of the various theoretical positions below in
