...
concerning the availability of local readings. Various problematic
...
\item K1:
  ...
  \end{itemize}
\item We also tested examples like (\ref{bsp:EA}), which are the ``global
alternatives'' to \es-sentences, with a universal quantifier where
\es-sentences have scalar \emph{some}.
\begin{exe}
...
\es-sentence. Interestingly, we found a difference in acceptability,
...
significant ($t = -3.8259$, $df = 322.375$, $p < 0.001$). That is a
...
proportional to the number of available true readings: if a putative
global reading is at stake, the acceptability pattern should be
reversed; if no such global reading is at stake, then the
acceptability difference cannot be explained.
\end{itemize}
