...
Moreover, the preference for this reading was constrained by prosody. When prosody 
supported a dispreferred \ec-reading, the number of \lc-decisions was decreased. In both 
\lc- and \ec-sequences, the number of \lc-answers was highest when this reading
...
phrasing supported the \ec-reading (52\%  vs. 56\%). The number 
...
\begin{figure}
\includegraphics[width=\textwidth]{../pictures/graph_ec.pdf}
...
For traditionalism, the absence of global readings \textit{per se} can be
explained in a straightforward manner, for instance, by proposing
that the extra assumptions needed to derive global readings are not 
standardly available (as in the weak variety of traditionalism).
Alternatively, traditionalists could argue that the task's rather high 
processing demands lead to a suppression of global readings, in line with the
...
implicature responses in sentence verification tasks. However, it then remains 
unclear why memory effects should be restricted to global readings.
As a result, approaches like the unrestricted variety of traditionalism thereby 
loose the possibility of accounting for local readings of \as-sentences.
\fn{Remember that what we called unrestricted traditionalism
...
explain local readings for \es-sentences. Finally, since our materials were 
presented auditorily, we controlled for potential effects of silent
prosody. Importantly, local readings were observed in the accented as well
as in the unaccented condition, thus countering previous traditionalist
suggestions that local readings are to be attributed to specific pragmatic 
mechanisms.\dn{Petra: das habe ich hier so von der Intro übernommen. Sowohl
dort als auch hier fände ich es interessant, kurz zu sehen, was für alternative
pragmatische Mechanismen das sind - geht das in Kürze? Den nächsten Satz habe ich
in Bezug zum vorhergehenden Absatz nicht verstanden} 

%showed on the neutral accentuation case as
%well. 
...
%grammaticalism would be saved in a straightforward way. 
%But is this an argument for grammaticalism contra traditionalism? No 
%it clearly is not. 

%In order to be a predictive theory, grammaticalism thus needs a selection
...
the observed data.\dn{Petra: dieser Absatz scheint mir sehr wichtig und gut - 
wäre es möglich, hier noch etwas ausführlicher drüber zu schreiben?} 
