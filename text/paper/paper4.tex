...
\emph{irrespective} of the order in which each reading could first be
judged in a given sequence. Finally, the incremental verification task
...
\es-sentences, independently of prosody, but required intonational
...
pattern of reading preferences is the same for \as- and \es
conditions, namely that literal readings are preferred over local
readings which are preferred over global readings. More concretely,
our analyses suggest the following preference/availability patterns
(bracketed readings are unattested, in the sense that they are either
unavailable or heavily dispreferred):
...
not predicted by neither traditionalism nor
grammaticalism. Traditionalism, if left on its own, fails to predict
local readings in the \es-conditions. A Grammaticalism that relies on
the strongest meaning hypothesis fails to predict the right preference
order and cannot explain the absence of answers indicative of global
readings. Let's look more closely at what is and is not surprising
under each theoretical position and then reflect on how each position
might be adapted do deal with the observations that it does not
predict.
...
``shallow interpretation'' of the non-monotonic quantifiers might save
traditionalism. 
...
\citet{GeurtsTielvan-Tiel2013:Scalar-expressi} \citep[see
also][]{Tielvan-Tiel2014:Quantity-Matter} in response to the data
...
with only some connections is contrasted presented alongside two
universally connected elements, the mean rate of agreement with a
suitable \es-sentence is significantly higher than when only one
universally connected element is present. So, maybe our \es-sequences
similarly provoked local readings because of an overemphasized visual
contrast between \emph{some but not all} and \emph{all}.
...
What might make this contrast-based explanation more plausible is the
observation that subjects found \es-sentences difficult. As discussed
in Section~\ref{sec:results}, there were a non-negligible number of
\emph{true} answer at step 2 in the \es-sequence. This can be
explained on the assumption that participants read the non-monotonic
quantifier \emph{exactly one} as a monotonic quantifier \emph{at least
  one}. Similar ``shallow interpretation'' of \emph{exactly one} would
explain the high number of local answers for \es-sentences (and for
\es-sentence alone). If participants processed the truth-value for an
\es-sentence by looking for a \emph{unique} element that could be
singled out somehow, they would need to unravel the \es-sequence up to
the last picture. At this step contrast effects are stronger and would
not need to compete with the proper semantic interpretation of
\emph{exactly one}. Together, pictorial contrast effects in symbiosis
with a ``shallow interpretation'' non-monotonic quantifiers could be
an alternative explanation of (\ref{bsp:observed-preferences}) for
traditionalism. 
...
A related objection is that it is not feasible to actually compare
...
should be set-up. Eventually, a full processing model, design for data
from incremental verification tasks would be needed. We cannot offer
such a model here, but can only the offer the model that we did use as
a first step in that direction. More importantly, though, the only
part of our conclusions that might plausibly change in
(\ref{bsp:observed-preferences}) for a different generating model is
that the 3 \emph{false} answers in the \as-conditions with neutral
intonation might support the conclusion that the local reading is
marginally attested. But this hypothetical effect will certainly be no
bigger than marginal, and so no genuinely different conclusions of
theoretical relevance should be expected.
...
predictions about possibly observable answer patterns in our task,
given concrete instances of relatives saliences of readings and other
