...
theoretical positions. \emph{Traditionalism} holds, in particular,
that local readings should not occur frequently unless marked by
special intonation. \emph{Grammaticalism} holds that local readings
are unproblematically available and, at least under the most prominent
of selection principles for candidate readings, even at least as
salient as the literal reading (see
Section~\ref{sec:grammaticalism}). To distinguish between these
opposing positions, it is therefore necessary to assess the relative
salience of readings for \as- and \es-sentences under conditions that
control for prosodic markedness.
...
\citet{ChemlaSpector2010:Experimental-Ev}, might well be confounded
with what van Tiel and Geurts call \emph{typicality effects} (see
...
data. Backed up by the \emph{strongest meaning hypothesis}
...
putative readings. In sum, our data shed doubt on both traditionalism
and grammaticalism in their currently most common forms. We conclude
the paper by some thoughts on how either position could be amended in
...
ones.\fn{We mention for clarity that although weak traditionalism does
  not predict global readings to be preferred, it might still predict
...
alternatives. Another proposal is that readings are the more preferred
the better they answer the contextually given question under
discussion
...
Section~\ref{sec:results} and Table~\ref{tab:count_data_CC} on
...
there is a slight variant of the hypothesized existentially closed
reading of \emph{exactly one}, that would explain \emph{true}
judgments at position 5 in an ``modern traditionalist'' manner. This
is a reading of \emph{exactly one} as ``there is a \emph{unique} group
with cardinality exactly one.'' Although this is admittedly only a
vague sketch of a possible line of explanation, it is not entirely
implausible that such a uniqueness requirement has participants look
for a distinguished referent, which they find on position 5. But if
the reading in question is about a unique discourse referent, we can
explain how the predicate \emph{is connected to some if its \dots} can
be pragmatically enriched in the desired fashion, along the lines of a
``modern traditionalist'' account that takes discourse effects into
account \citep[][Chapter~8.4]{Geurts2010:Quantity-Implic}. The
listener would then conclude that the referent in question was not
connected to all of its surrounding elements, because otherwise the
speaker would have attributed that to the now fixed referent in
question. This is parallel, so Geurts suggests, to the reasoning that
an utterance of ``Smith met a woman'' implicates that \emph{the woman
  referred to} was not Smith's mother and not that Smith didn't meet
his mother at all. But this alternative explanation is at present only
a tentative sketch of a possibility.

In conclusion, then, it appears that our data provides interesting
