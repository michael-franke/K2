...
Our previous regression-based analyses did not take the sequential
...
target-related controls suggests otherwise. In order to estimate this
bias and the preference for candidate readings, we use a simple
Bayesian cognitive model that computes the likelihood of answer
patterns for different latent sequential biases and reading
preferences. We use the data to estimate the posterior likelihood of
these latent parameters to gain insight into the likely preference
relations and the salience of local readings.
...
to the sequential bias, not a difference in general, task-independent
salience of readings.

To add potential effects of sequentiality, we assume that there is a
constant bias factor $q \in [0;1]$. This bias is held constant over
all conditions, because we assume that there is a global tendency to
answer early or late, independent of the sentence subjects are
confronted with. The bias factor captures whether there is a tendency
to answer early ($q > .5$) or later ($q <.5$) in the sequence. In the
...
and that $p_{\text{glb}}$ is multiplied by $(1-q)^2$.

Finally, we also allow for the chance of subjects making mistakes. To
keep things as simple as possible (no consideration for spill-overs,
late decisions, preferences to unravel completely before judging
etc.), we assume that there is a fixed error rate $e^k \in [0;1]$ for
each sentence-intonation pair $k$. We allow for different error
rates for different sentence-intonation pairs, because we want to
allow for the possibility that non-monotonic quantifiers might be
harder to process, or that some kind of intonation (like the \ec-cue)
is harder than others. A decision can be wrong but incidentally
coincide with a reading that is indicative of a relevant
...
Taking this together we calculate, for each critical
sentence-intonation pair $k$ a target probability vector $\myvec{t}^k
=
