...
\begin{table}
  ...
  \begin{tabular}{lllll}
    ...
    \midrule
    \textsc{Intercept} & 1.679  &  0.134 &  12.499  & < 0.001 \\
    \textsc{Reading}.\emph{literal} & -0.219  &  0.281 & -0.781 &
    0.435 \\
    \textsc{Reading}.\emph{local} & -2.249 &   0.172 & -13.084 & < 0.001 \\
    \textsc{Condition}.\es & 0.592 &  0.159 & 3.732 &  < 0.001 \\
    \textsc{Reading}.\emph{literal}:\textsc{Condition}.\es & -0.0186 &
    0.328 & -0.057 &  0.95471 \\
    \textsc{Reading}.\emph{local}:\textsc{Condition}.\es & 1.041 &
    0.208 &  4.999 &  < 0.001
  \end{tabular}
  \caption{Coefficients of the ``best'' log-linear model for the count
    data in Table~\ref{tab:CC-ContingencyTable}.}
...
\textsc{Reading} might not be necessary. In order to test whether
there is support for the postulation of local readings, we therefore
compared the previous ``best'' predictor to a model that only differs
from the former in that it subsumes the level \emph{local} of factor
...
level \emph{local} (df=30, residual deviance = 9.76) over the simpler
