...
participant due to insufficient performance on controls ($\le 60\%$
correct answers).
...
complement the picture, we also provide a Bayesian analysis that
...
\caption{Positional judgments for the four target conditions. Bars
  ...
  total of 103 \emph{yes} judgments at the third position in the
  \as-sequence with neutral intonation, which corresponds to 103
...
in Table~\ref{tab:CC-ContingencyTable}. The table also classifies
...
\as-conditions, but 19.30\% / 17.54\% 'error' in the neutral /
accented \es-conditions. A look at Figure~\ref{fig:JudgmentsK2}
reveals that there is some systematicity in 'error' answers. Most
answers that we classified as 'errors' are plausibly explained as a
few random errors, a handful of mistakes resulting from confusing
buttons for \emph{true} and \emph{false} and a few 'spill-overs' where
participants gave responses one position too late. There is one
notable exception, though. A substantial number of \emph{true}
responses where given at the second position of \es-sequences (9.65\%
/ 8.77\%) which is shown in Figure~\ref{fig:exseqES2}. Although this
position in the sequence is the critical position to reveal a global
reading, the latter would be indicated by a \emph{false} response. It
appears that subjects read \es-sentences as if the non-monotonic
quantifier \emph{exactly one} was a non-monotonic \emph{at least one},
...
were single answers by 5 different subjects. \todo{subjects 1, 9, 12,
  21, 84} This suggests that at least for some subjects non-monotonic
quantifiers were problematic. \todo{how to deal with this in the future?}
...
predict the dependent factor \emph{Response} with levels
\emph{literal}, \emph{local}, and \emph{other}. The latter levels
lumps \emph{global} and \emph{error} responses together, because
otherwise we would not have enough cell counts to apply log-linear
...
\emph{Accent} are spurious in the best model. This suggests that
...
In order to test whether there is support for the postulation of local
readings, we compared the previous ``best'' model to a model that only
differs from the former in that it subsumes the level \emph{local} of
factor \textsc{Reading} under level \emph{other} as well
...
level \emph{local} (df=30, residual deviance = 9.76) over the leaner
model without this level (df=32, residual deviance = 12.39,
AIC=156.63). This means that our data does not provide strong evidence
for the maintenance of belief in \emph{local} readings in our
setting. (Subsequent analysis will shed more light on this.)


\paragraph{Preference-related controls.} Positional answers for our
preference-related control conditions are shown in
Figure~\ref{Fig:TF_judgments}.
%
\begin{figure}[]
\centering
...
\caption{Positional answers for \lc-sequences and
  \ec-sequences. \emph{True} judgements are plotted upwards on each
  position, \emph{false} judgements downwards.}
...
distributions of readings thus coded, for each position at which this
critical item occurred during the experiment, is given in
Table~\ref{tab:CC-ContingencyTable-TF}.
...
\begin{tabular}{lcccc}
  ...
  & Pos 1 & 14 &  18 & 3  \\
  & Pos 2 & 16 &  16 & 1  \\
  & Pos 3 & 14 &  17 & 5  \\ \addlinespace[0.15cm]
  & \textbf{Total} & \textbf{99} &  \textbf{8} & \textbf{2}  \\
  \addlinespace[0.25cm]
  \textbf{\ec neutral} 
  & Pos 1 & 22 &  6 & 10  \\
  & Pos 2 & 28 &  6 & 4  \\
  & Pos 3 & 28 &  8 & 2  \\ \addlinespace[0.15cm]
  & \textbf{Total} & \textbf{99} &  \textbf{8} & \textbf{2}  \\
  \addlinespace[0.25cm]
  \textbf{\ec with \lc cue} 
  & Pos 1 & 30 &  5 & 3  \\
  & Pos 2 & 32 &  5 & 1  \\
  & Pos 3 & 30 &  3 & 2  \\ \addlinespace[0.15cm]
  & \textbf{Total} & \textbf{99} &  \textbf{8} & \textbf{2}  \\
  \addlinespace[0.25cm]
  \textbf{\ec with \ec cue} 
  & Pos 1 & 19 &  10 & 9  \\
  & Pos 2 & 19 &  7  & 12  \\
  & Pos 3 & 19 &  7  & 12  \\ \addlinespace[0.15cm]
  & \textbf{Total} & \textbf{99} &  \textbf{8} & \textbf{2}  \\
\end{tabular}
...
Judgments were
coded as \ec or as \lc if they were as expected under one of these
readings, and as "other" if they were not. The distributions of
readings thus coded is illustrated in Figure
\ref{fig:response-type-freque-TRF}.  Overall, participants mainly gave
answers corresponding to the \lc-reading (65\%).  Moreover, this
pattern was constrained by prosody. When prosody supported a
dispreferred \ec-reading, the number of \ec-decisions were increased
and vice versa.
