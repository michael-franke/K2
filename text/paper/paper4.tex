...
and the demarkation between semantics and pragmatics. The controversy
...
main theoretical positions can be distinguished here:
\emph{traditionalism} holds in particular that local readings should
not occur frequently unless marked by special intonation;
...
distinguish between these opposing positions, it is necessary to
assess the availability and relative preference of readings for \as-
and \es-sentences under conditions that control for prosodic
markedness. 
...
\es-conditions respectively. A look at the right-hand side of
...
If we compare (\ref{bsp:observed-preferences}) with
Table~\ref{tab:predictions}, we quickly see that these findings are
not predicted by either traditionalism or
grammaticalism. We address each position separately in the following.
...
(\ref{bsp:observed-preferences}). Again, we need to bare the caveat of
