...
\begin{abstract}
  ...
  experimental sheds light on the relative preferences or salience of
  candidate readings. The presented data turns out to be problematic
...
scope of other logical operators. This paper deals with two such cases.
In \as-sentences (mnemonic for \textit{all \dots some \dots}) as in
(\ref{bsp:AE}) the scalar item \emph{some} is embedded under universal
quantifier \emph{all}. In \es-sentences (mnemonic for \textit{exactly
  one \dots some \dots}) like (\ref{bsp:GE}) \emph{some} takes scope
under the non-monotonic quantifier \emph{exactly one}.  According to
current pragmatic theory, there are at least three relevant candidate
readings for \as- and \es-sentences: (i) a \emph{literal reading} like
in (\ref{bsp:AE-Literal}) and (\ref{bsp:GE-Literal}) where \emph{some}
...
(\ref{bsp:AE}) is enriched and (\ref{bsp:GE}) with the negation of the
...
how salient or preferred are these theoretically conceivable readings
...
inferences and the demarkation between semantics and pragmatics. The
controversy is usually seen as one between two main competing
theoretical positions. \emph{Traditionalism} holds in particular that
local readings should not occur frequently unless marked by special
intonation. \emph{Grammaticalism} holds that local readings are
unproblematically available and, at least under the most prominent of
selection principles for candidate readings, even at least as salient
as the literal reading (see Section~\ref{sec:grammaticalism}). To
distinguish between these opposing positions, it is therefore
necessary to assess the relative salience of readings for \as- and
\es-sentences under conditions that control for prosodic markedness.


A number of empirical studies have already examined sentences like
(\ref{bsp:AE}) and (\ref{bsp:GE})
\citep[e.g.][]{GeurtsPouscoulous2009:Embedded-Implic,CliftonDube2010:Embedded-Implic,ChemlaSpector2010:Experimental-Ev,BenzGotzner2014:Embedded-implic}. Unfortunately,
results from these studies do not provide uncontroversial
evidence. This is due to several reasons. Firstly, previous studies
have only accumulated limited evidence pertaining to the relative
salience of candidate readings of both types of sentences. Secondly,
as \citet{Tielvan-Tiel2012:Embedded-Scalar} and
...
\citet{ChemlaSpector2010:Experimental-Ev}, might well be confounds of
what van Tiel and Geurts call \emph{typicality effects} (see
Section~\ref{sec:previous-studies} for discussion). Finally, and most
importantly, previous studies presented target sentences
visually. However, traditionalists acknowledge the availability of
local readings for prosodically marked utterances
\citep[e.g.][]{Horn2006:The-Border-Wars,Geurts2009:Scalar-Implicat,ChemlaSpector2010:Experimental-Ev,Geurts2010:Quantity-Implic,Tielvan-Tiel2012:Embedded-Scalar,GeurtsTielvan-Tiel2013:Embedded-Scalar}. We
will address this extant position as the \emph{prosodic markedness
  hypothesis} and argue that, if intonation is hypothesized to play a
central role, it is vital to present target sentences auditorily, and
to control for effects of ``silent prosody'' that subjects may apply
when reading a sentence \citep[see][]{Bader98,Fodor98}.

This paper presents results form an experimental study designed to
deal with the above mentioned problems. We presented sentence
materials auditorily and explicitly manipulated intonational stress on
embedded scalar items. To cater for potential pictorial confounds,
such as from typicality, we employed an \emph{incremental verification
...
data that is both more clearly indicative of the reading that subjects
assumed and more independent of any potential pictorial confounds, such
as the mentioned typicality effects. Moreover, this method also gives
information about reading preferences. We validated this by including
control conditions with ambiguous sentences with a well-known
preference on available readings.

The data obtained by this method is \emph{prima facie} problematic for
both traditionalism and grammaticalism. Contrary to the predictions of
traditionalism, we observed many answers indicative of local readings
of \es-sentences, irrespective of intonational markedness. But also
grammaticalism is incongruent with the observed pattern in our
data. Backed up by the strongest meaning hypothesis
...
grammaticalism predicts local readings to be the preferred readings,
but we found no evidence for that. More strongly even, the preference
...
putative readings. In sum, our data sheds doubt on both traditionalism
and grammaticalism in their currently most common forms. We conclude
the paper by some speculation about how either position might be
amended in the light of our data.
...
some leeway in assessing the predictions for each approach (depending
on which of several additional assumptions to adopt), we will
distinguish different varieties of both of them. Predictions of all
relevant variants are summarized in Table~\ref{tab:predictions} on
...
readings, because these form the starting point of pragmatic
reasoning; global readings because these may be arrived at by the
Gricean recipe.
...
implausible, traditionalism would predict that literal readings would
be preferred over global ones. Consequently, there are at least two
varieties of traditionalism that, depending on which additional
...
auxiliary assumptions of the Gricean recipe hold (e.g., as a matter of
strong default) and therefore predicts that global readings are
preferred over literal ones; \mymark{the weak variety of
  traditionalism} holds that the auxiliary assumptions are more
fragile and predicts that literal readings are preferred over global
ones.\fn{We mention for clarity that although weak traditionalism does
  not predict global readings to be preferred, it might still
  predict an \emph{epistemically weak implicature}, similar to the
...
As far as preferences are concerned, even unrestricted traditionalism
would not predict that the local reading is preferred over the global
reading. This is because the derivation of the local reading via
...
The grammatical approach, as described so far, is not yet a fully
articulated theory. To make concrete predictions, it must be
supplemented by a selection principle that determines which of the
...
alternatives. Another proposal is that readings are preferred in
relation to how well they answer the contextually given question under
discussion
...
is prosodically marked by contrastive stress
...
To date, empirical evidence for this assumption is still scarce. One
notable exception comes from a pilot study reported in
...
the authors found a significant impact of overtly realized contrastive
accents on implicature processing: Besides a general advantage for
...
obvious way \citep[c.f.][for critical reflection on typicality-based
explanations]{Cummins2014:Typicality-made}. Moreover, typicality
itself is not a satisfactory primitive in a putative explanation of
the use of quantifiers. Rather it is possible to explain typicality
judgements as the outcome of Gricean speaker preferences for
informative descriptions, just like those that rationalize quantity
implicatures \citep{Franke2014:Typical-use-of-}. For these reasons, we
believe that typicality and other pictorial effects need to be taken
seriously in the experimental design, but do not give a satisfactory
account of the observed variation all by themselves.
...
that subjects are requested to judge sentence material with respect to
pictures that do not necessarily contain all the information relevant
for judging a certain reading true or false. In that case,
participants can demand that more information be
...
relations among multiple candidate readings. Finally, these conditions
also controlled whether prosodic information can, in principle, shift
answer patterns in the present task. Note that for our target
...
\end{exe}
In late-closure sentences, a modifier like a relative clause or a
...
\noindent Out sentences were read by a phonetically trained female
...
an \lc-reading. Thus, a phrase boundary between {\it squares} and {\it
  with} in (\ref{bsp:target-related}) supports an \ec-reading, whereas
a prosodic phrase boundary following {\it circles} supports an
\lc-reading.
...
the number of \lc- or \ec-readings for each intonation type across
...
taken as indicative of \lc-readings. Finally, intonation patterns that
cue dispreferred \ec-readings seem to have led to more mistakes than
...
\textsc{Trial} again turned out to be spurious, suggesting that
...
\begin{figure}
  ...
  \hfill
  \subfloat[][accented]{
      \includegraphics[width=0.45\textwidth]{PostDiffT_ASacc.pdf}
  }
...
available or only allow for epistemic weak implicatures that would
...
options might be to account for local readings as the result of some
other phenomenon, such as typicality or contrast
...
A more promising alternative explanation for local readings of
\es-sentences is pursued by
...
On the other hand, there is a conceivable ``modern traditionalist''
alternative explanation of the puzzling data for \es-sentences that
would also account for the large and systematic error
responses. Recall from Section~\ref{sec:results} and
Table~\ref{tab:count_data_CC} on \pageref{tab:count_data_CC} that a
surprisingly high number of \emph{true} answers was given at the
second step of the \es-sequence. This answer does not correspond to
any of the three candidate readings that the previous literature has
focused on. We hypothesized in Section~\ref{sec:results} that these
readings can be explained under the assumption that \emph{exactly one}
gets a reading similar to \emph{there is (at least) one}, while the
scalar item \emph{some} would just receive its semantic
interpretation. Such a reading of \emph{exactly one}, although
surprising, could actually be supported by some theories of numerals
and modifiers
...
sense of \citet{Partee1986:Noun-phrase-int} to the effect that
...
construal does not explain \emph{true} judgements at position 5. These
would therefore still seem indicative of local readings. However,
under the hypothesized existentially closed reading of \emph{exactly
  one}, a ``modern traditionalist'' approach that takes discourse
effects into account \citep[][Chapter~8.4]{Geurts2010:Quantity-Implic}
would predict the availability of a pragmatically strengthened reading
of \es-sentences of the form: ``there is a group of cardinality exactly
one and that group has the property that its members are connected to
some but not all of their surrounding elements.'' This would explain
the observed patterns in \es-conditions. Participants who read
\emph{exactly one} as a non-monotonic quantifier would, pace
traditionalist logic, prefer a literal interpretation and answer
\emph{false} at step 3 unless they made a mistake. Participants who
assumed an existentially-closed quantifier reading could, according to
a discourse-sensitive ``modern traditionalism'', apply a literal or a
pragmatically strengthened reading. In the former case, they would
answer \emph{true} at step 2, in the latter case they would answer
\emph{true} at step 5, without this being necessarily a ``local
reading'' in the sense put forward by proponents of grammaticalism. 

This alternative explanation is not without problems and is not backed
up by a full account of discourse-based pragmatic reasoning. In
conclusion, then, it appears that our data provides interesting
challenges to both traditionalism and grammaticalism.
