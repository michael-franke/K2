...
readings, we compared the previous ``best'' predictor to a model that
only differs from the former in that it subsumes the level
...
that includes the level \emph{local} (df=30, residual deviance = 9.76)
over the simpler model without this level (df=32, residual deviance =
12.39, AIC=156.63). 
...
setting. But we need to be careful. So far we have not taken into
account the sequential structure of our task. It might be that local
readings are infrequent because subjects tend to exit early in the
sequence, rather than late. This is why we need preference-related
controls.
...
Figure~\ref{Fig:TF_judgments}.\fn{We removed 19 data points for the
  ...
  classifying them as \emph{errors} does not change the results of the
  reported analyses.}
%
\begin{figure}[]
\centering
...
\begin{tabular}{lcccc}
  ...
  & \textbf{Total} & \textbf{76} &  \textbf{27} & \textbf{1}  \\
  \addlinespace[0.25cm]
...
intonation ups the count for \lc-readings, while \ec-cueing ups the
count for \ec-readings. If we compare the number of \lc- or
\ec-readings for each intonation type across sequences, we see that
there are always more answers indicative of the relevant readings when
that reading can be judged later in the sequence.

There are only very few answers that appear to be random
mistakes. Some answers in the `error' category are plausibly
`spill-over' errors. But on top of that, a substantial number of
`error' answers occur on critical positions. In the \lc-sequence there
...
taken as indicative of \lc-readings. \todo{A general tendency to
  answer later in the sequence does explain this. We don't see a
  similar effect in the critical conditions because the literal
  reading is so dominant.} Finally, intonation patterns that cue
\ec-readings seem to have led to more mistakes than other intonation
patterns.

\begin{itemize}
\item \textcolor{red}{LOG-LINEAR MODELS}
\end{itemize}
