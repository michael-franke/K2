...
scope of other logical operators. This paper deals with to such cases.
...
\noindent Given this plurality of hypothesized readings, the question
arises:

\begin{exe}
\ex How salient or preferred are these theoretically conceivable
  readings compared to each other (if they are detectable by empirical
  means at all)?  
\end{exe}

\noindent Addressing this question empirically is relevant because it
...
necessary to assess the empirical observability and relative
preference of readings for \as- and \es-sentences under conditions
that control for prosodic markedness.
...
have only accumulated limited evidence pertaining to the preference
relations between candidate readings of both types of
sentences. Secondly, as \citet{Tielvan-Tiel2012:Embedded-Scalar} and
...
visually. However, traditionalists admit to the availability of local
readings as long as the utterance is prosodically marked
...
will address this extant position as the \emph{prosodic markedness
  hypothesis}. If intonation plays a role, it is vital to present
target sentences auditorily, and to control for effects of ``silent
prosody'' that subjects may apply when reading a sentence
...
assumed and mor independent of any potential pictorial confounds, such
...
% traditionalism and grammaticalism. Contrary to the predictions of
...
\citep[e.g.][]{Augurzky08,Bader98,Fodor98,Fodor02,Kentner12,
  Steinhauer01}. If we adopt the prosodic markedness hypothesis
...
7.0\% / 4.4\% unclassifiable judgements in the neutral / accented
...
plausible explanation: random errors, a handful of mistakes
...
substantial number of \emph{true} responses were given at the second
position of \es-sequences (9.7\% / 8.8\%) which is shown in
...
preference/availability patterns (bracketed readings are unattested,
in the sense that they are either unavailable or heavily
dispreferred):
...
well as in unaccented \es-sentences. It would also remain unclear why
...
grammaticalism with respect to a particular selection criterion
here. The verdict, however, seems to be clear. If
(\ref{bsp:observed-preferences}) is the actual preference order over
readings, then meaning selection in terms of logical strength is
...
traditionalist framework is difficult, if possible at all. A common
approach is to account for local readings as the result of some other
...
\es-sentences, but that the additional assumption of ``shallow
interpretation'' of the non-monotonic quantifiers might save
traditionalism.
...
to be implausible. But even if this line of explanation can be
plausibly extended to incrementally revealed picture verification, it
would still be unclear why typicality-effects should show so strongly
in categorical truth-value judgements, and why there is such a stark
contrast between \as- and \es-sentences.
...
On the other hand, what might make this contrast-based explanation
more plausible is the observation that subjects had difficulties with
\es-sentences. As discussed in Section~\ref{sec:results}, there were a
non-negligible number of \emph{true} answer at step 2 in the
\es-sequence. This can be explained on the assumption that
participants read the non-monotonic quantifier \emph{exactly one} as a
monotonic quantifier \emph{at least one}. Similar ``shallow
interpretation'' of \emph{exactly one} would explain the high number
of local answers for \es-sentences (and for \es-sentence alone). If
participants processed the truth-value for an \es-sentence by looking
for a \emph{unique} element that could be singled out by a description
like ``\dots is connected to some of its \dots'', they would need to
unravel the \es-sequence up to the last picture where the relevant
visual contrast is stronger. A ``shallow interpretation''-theory also
sidesteps the puzzle why answers are given that are incompatible with
the proper semantic meaning of \es-sentences. Together, pictorial
contrast effects in symbiosis with a ``shallow interpretation'' of
non-monotonic quantifiers could be an alternative explanation of
(\ref{bsp:observed-preferences}) for traditionalism.
