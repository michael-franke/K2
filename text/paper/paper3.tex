...
\begin{abstract}
  Scalar item \emph{some} is widely assumed to undergo, given relevant
  preconditions, a pragmatic meaning enrichment to \emph{some but not
    all} if it occurs in matrix position. The question under which
  circumstances this meaning enrichment can take place in embedded
  position plays an important role in deciding how to delineate
  semantics and pragmatics. % Some claim
  ...
  % freely.
  We present a novel experimental paradigm to test whether the
  relevant pragmatic enrichment is available in embedded positions
  with or without prosodic markedness. On top of adding prosodic
  manipulation, our experimental design was catered to a number of
  methodological difficulties that were identified for previous
  related studies. The data obtained refutes both a traditional
  Gricean view of scalar implicature calculation, as well as the more
  recent grammatical approach.
\end{abstract}
...
for. This is due to several reasons. Firstly, previous studies have
only accumulated limited evidence pertaining to the preference
relations between putative readings of both types of
...
data that is both indicative of the reading subjects assumed and
independent of any potential pictorial confounds. We also made sure
that our method is able to reveal information about reading
preferences by including a control condition with ambiguous sentences
with a well-known preference on available readings.
...
the intonational markedness hypothesis on its own is not supported by
...
and preference of readings of our target
sentences. Section~\ref{sec:previous-studies} recaps the results of
related experimental studies to the extent necessary to motivate our
own approach. Section~\ref{sec:exp} eventually contributes our
...
\citep[see][]{Geurts2010:Quantity-Implic} for deriving a simple
...
provided by the global reading, mutual awareness that the stronger
...
speaker knows exactly how many problems Hans solved. In that case, or
if any other assumption of the Gricean recipe cannot be maintained,
...
\citep[see][]{GeurtsPouscoulous2009:Embedded-Implic,ChemlaSpector2010:Experimental-Ev}. This
...
Another prominent theoretical position is
...
\citep[see][]{Chierchia2006:Broaden-Your-Vi,Fox2007:Free-Choice-and,Magri2011:Another-Argumen,Sauerland2012:The-Computation,ChierchiaFox2008:The-Grammatical,Chierchia2012:FC-Nominals-and}. % When
...
meaning of the form:\fn{More sophisticated formulations of exhaustive
  ...
  semantics \citep[see][]{Chierchia2006:Broaden-Your-Vi}.}
\begin{exe}
...
\citep[see][]{FoxSpector:Economy-and-Emb,ChierchiaFox2008:The-Grammatical,Chierchia2012:FC-Nominals-and}
...
On the other hand, \citet{Crain1998} argues in
favor of truth-value judgments as a means of detecting dispreferred
readings. Consequently, traditionalists could reply that the putative
evidence for local readings in non-categorical judgement tasks might
reflect something other than (strongly) truth-relevant
speaker-intended meaning enrichments. Hence no case can be made for
lexicalism or grammaticalism on the basis of these data
\citep[see][for arguments along these
...
\citep[see][]{DegenTanenhaus2012:Processing-Scal}. For these reasons,
...
readings. In addition, we wanted to minimize all effects of alleged
typicality of pictures. To achieve this, we used an
\mymark{incremental verification task} (\acro{ivt}), which is a
modified version of picture verification \citep[see][]{Conroy2008}. The
general idea is that subjects are requested to judge sentence material
based on pictures that do not necessarily contain all the information
necessary to judge a certain reading true or false. In that case,
...
uncovered, and it is now clear that at least one of the letters is not
...
thus yielding \emph{false} answers if participants indeed adopted
...
uniquely to candidate readings. All other \emph{true} or \emph{false}
answers were counted as errors. The mapping between responses and
...
of the other bells are linked to all of them. Note that global and
literal readings again differ from local readings with respect to
their truth values. Mappings between responses and readings are
summarized in Table \ref{tab:mappingES}.


\paragraph{Positional controls.} In order to make sure that subjects
understood the task, i.e., gave truth-value judgements at the first
possible position in a sequence, we included a set of control
conditions. These also controlled for response biases. For each of the
...
\acro{pp} \emph{with suns} can be attached to the preceding
...
For each such sentence, two sequences were designed. In the first one, the
...
(\ref{bsp:target-related2}), one for each sentence type, for proper
comparison. 
...
presented using active PC loud speakers and the picture materials were
...
information, (iii) respond "yes, fits" or (iv) "no, does not
fit". At the beginning of an experimental session participants
received written instructions on their task. Then they completed an
interactive training consisting of 15 trials in which they were
familiarized with the task.  After the training the actual experiment
started. Participants were tested individually in a silent room. Each
session was divided into three blocks and participants were told they
could take breaks between blocks. In total, an experimental session
took about one hour. At the end of the session participants received 8
euros compensation.
...
(\ref{ex:es}). Sentences were introduced by a subject {\acro{dp}}, including the quantifiers {\it alle diese} ("all of
...
bells. Subjects were followed by an auxiliary, and a {
...
these sentences, subject \acros{dp} were always denoting
icons and were followed by an auxiliary. In the ambiguous sentences,
the auxiliary was followed by two {\it mit}-\acros{pp}.  The
first of these \acros{pp} consisted in a coordination of two
...
reveal that all prosodic realizations (neutral vs. (\emph{early
...
like in Figures~\ref{fig:InitialPicture}--\ref{fig:exlc}. In each
picture, four alternative sub-scenarios were presented, in which an
icon was surrounded by six geometrical shapes. Depending on the
...
sequence of the preference-related control structures. Grey colour
initially covered the links between icons and shapes. As the
connections were to be uncovered step by step, different pictures were
designed corresponding to different stages in the uncovering
process. Inherent entailment relations between \as-sentences and
\es-sentences lead to different critical positions at which
participants can make their judgments. We will therefore describe the
sequences for each condition separately.
...
critical positions. The critical positions (Figure \ref{fig:exseqES})
are associated with the second picture (global reading), the third
picture (literal reading), as well as with the final picture (local
reading).
...
position for the early-closure reading (\LC-sequence), and second, a
sequence in which the readings were presented in reversed order
(\EC-sequence). In the \LC-sequences, seven pictures were presented,
with the \LC-reading given at the third picture, and the \EC- reading
at the sixth picture. \EC-sequences consisted of five pictures, in
which the critical positions were realized at the third picture
(\EC-reading) and at the fifth picture (\LC-reading).
...
Performance on positional controls was overwhelmingly correct (92\% correct responses on
average) indicating that participants understood the task and were able to give
correct truth-value judgments at the adequate point in a sequence. 
In general, they were not affected by inessential features of the picture materials
...
observed. In the \as-conditions (see
Figure~\ref{fig:JudgementPercentagesAE}), judgments were consistent
with literal readings in 65.0\% of the trials with neutral prosody and
in 67.5\% of the trials with accented prosody. Judgments indicating
local readings were given in 22.0\% of both the neutral and accented
trials. Global readings occurred in 1.6\% of the neutral and 0.8\% of
the accented trials (corresponding to a total of 3 answers across all
trials).  In the \es-conditions (see
...
neutral and accented prosody, respectively.  Judgments indicating
local readings were given in 22.8\% of the neutral and 27.6\% of the
accented trials. Global readings were observed in 0.8\% of the trials
in both the neutral and the accented condition.  As for the number of
erroneous decisions, a discrepancy between \as-conditions and
\es-conditions was observed: In the former, participants made hardly
any errors, while in the latter, the amount of errors was considerably
larger.

Interestingly, there was considerable variation of response patterns within participants. The relative frequencies of local responses seemed to be distributed bimodally (see Figure \ref{fig:HistogramLocalReadingsK2}). We classified participants as {\it localists}, {\it globalists} or {\it inconsitent}. Table \ref{table:answer-types-as} shows how many of each type we observed. Further examination of the distribution of local readings revealed a clear pattern. The relative frequency of local judgments per participant strongly
correlated between all four conditions (all $r>.6, p<.001$). Within the non-localists, about half of the participants (41.5\% of all
participants) were consistently giving judgments that indicate literal readings. Only a minority of participants exhibited inconsistency. 
...
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,0) (4,0) (5,64)};

        \addplot coordinates {(1,4) (2,2) (3,26) (4,1) (5,5)};
...
\subfloat[][prosody cues \lc-reading]{
% \includegraphics[width=5cm]{../pictures/paper/graph_GE_NTR.jpg}
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,0) (4,0) (5,76)};

        \addplot coordinates {(1,4) (2,0) (3,13) (4,2) (5,5)};
...
\subfloat[][prosody cues \ec-reading]{
% \includegraphics[width=5cm]{../pictures/paper/graph_AE_AKZ.jpg}
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,0) (4,0) (5,56)};

        \addplot coordinates {(1,2) (2,1) (3,33) (4,2) (5,6)};
...
\subfloat[][unambiguous \lc-reading]{
% \includegraphics[width=5cm]{../pictures/paper/graph_GE_AKZ.jpg}
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,1) (2,0) (3,0) (4,1) (5,86)};

        \addplot coordinates {(1,6) (2,0) (3,5) (4,0) (5,2)};
...
\caption{Judgments for preference-related controls (EC
  sequence). Rounded percentages of {\it yes/no} judgments are plotted
  against the position within the trial. Each EC sequence consisted of
  five pictures, in which the critical positions for a reading were
  the third position (\emph{no} corresponded to an \ec reading) and
  the fifth position (\emph{yes} corresponded to an \lc reading).}
...
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,68) (4,0) (5,0) (6,1) (7,0)};

        \addplot coordinates {(1,0) (2,0) (3,5) (4,0) (5,0) (6,25) (7,2)};
...
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,79) (4,1) (5,1) (6,2) (7,1)};

        \addplot coordinates {(1,0) (2,1) (3,3) (4,0) (5,0) (6,13) (7,0)};
...
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,51) (4,2) (5,1) (6,4) (7,0)};

        \addplot coordinates {(1,2) (2,0) (3,8) (4,1) (5,0) (6,30) (7,2)};
...
\begin{tikzpicture}[scale=0.75]
      ...
      \begin{axis}[ybar, 
        ...
        \addplot coordinates {(1,0) (2,0) (3,88) (4,2) (5,0) (6,5) (7,0)};

        \addplot coordinates {(1,1) (2,0) (3,1) (4,0) (5,0) (6,3) (7,0)};
...
\emph{lexicalism}) proposes that if the upper-bounding inference from
